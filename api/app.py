...
# loggingBaseConfig = {
#     'version': 1,
#     'formatters': {'default': dict(format='[%(asctime)s] %(levelname)s in %(module)s.%(funcName)s [%(pathname)s:%(lineno)d]: %(message)s')},

#                     # 'uwsgi': dict(format='%(addr) - %(user) [%(ltime)] "%(method) %(uri) %(proto)" %(status) %(size) "%(referer)" "%(uagent)"')},
#     'handlers': {
#         'wsgi': {
