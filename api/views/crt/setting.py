...
class AcmeSettingView(APIView):
    ...
    def post(self):
        ...
        elif Acme.query.filter(db.exists().where(and_(Acme.user==user,Acme.deleted_by.is_(None),Acme.acme_type_id==AcmeType.get_by(name=acme_type,to_dict=False,first=True).id))).scalar():
